...
/**
 * Legacy default preview length. The authoritative value now lives on
 * `LlmTuiConfig.toolPreviewChars`; callers pass an explicit `max` into
 * `defaultCollapsedSummary` / `defaultResultPreview` to override.
 */
export const MAX_PREVIEW = DEFAULT_CONFIG.toolPreviewChars;
...
export function defaultCollapsedSummary(args: unknown, max: number = MAX_PREVIEW): string {
  if (args == null) return "";
  if (typeof args === "string") return truncate(compactWhitespace(args), max);
  if (typeof args !== "object") return truncate(String(args), max);
  return truncate(pickPrimary(args as Record<string, unknown>, PRIMARY_ARG_KEYS), max);
}
...
export function defaultResultPreview(result: unknown, max: number = MAX_PREVIEW): string {
  if (result == null) return "";
  if (typeof result === "object") {
    return truncate(pickPrimary(result as Record<string, unknown>, PRIMARY_RESULT_KEYS), max);
  }
  if (typeof result !== "string") return truncate(String(result), max);
  if (result.length === 0) return "";

  const extracted = extractPrimaryString(result, PRIMARY_RESULT_KEYS);
  const s = extracted ?? result;
  if (s.length === 0) return "";
  return truncate(compactWhitespace(s), max);
}
