...
const plugin: KaizenPlugin = {
  ...
  async setup(ctx) {
    ...
    // Config: backed by kaizen-config. Harness manifest may seed theme
    // defaults via ctx.config.theme. /config:set llm-tui <field>=<value>
    // updates live (theme fields apply on the next render; the completion
    // debounce field is captured at construction and requires a restart).
    const harnessDefaults =
      ((ctx.config as { theme?: Partial<UiTheme> } | undefined)?.theme ?? {}) as Partial<UiTheme>;

    let currentConfig: LlmTuiConfig = { ...DEFAULT_CONFIG, ...BUILT_IN_THEME, ...harnessDefaults };

    // Store first — the watch callback below needs to push into store.setConfig().
    const store = new TuiStore({ theme: currentConfig, config: currentConfig });
    ...
    try {
      ...
      cfgStore.register<LlmTuiConfig>({
        plugin: "llm-tui",
        defaults: { ...currentConfig },
        schema: CONFIG_SCHEMA,
      });
      currentConfig = cfgStore.get<LlmTuiConfig>("llm-tui");
      store.setConfig(currentConfig);
      teardownConfigWatch = cfgStore.watch<LlmTuiConfig>("llm-tui", (next) => {
        try {
          currentConfig = next;
          store.setConfig(next);
        } catch (err) {
          ctx.log(`llm-tui: failed to apply config update: ${(err as Error).message}`);
        }
      });
    } catch (err) {
    ...
    const themeService: UiThemeService = { current: () => currentConfig };
    ctx.provideService<UiThemeService>("ui:theme", themeService);

    // Store + completion registry are shared between the channel + UI. The
    // completion debounce value is read once here at construction; live
    // updates to `completionDebounceMs` require a restart.
    const registry = makeCompletionRegistry({ debounceMs: currentConfig.completionDebounceMs });
    ctx.provideService<UiCompletionService>("ui:completion-source", registry.service);
    const toolRenderers = makeToolRendererRegistry({
      getFallbackJsonChars: () => currentConfig.toolFallbackJsonChars,
    });
    ...
    // renderer with the same toolName. The getConfig closure is read on every
    // render so live config-store updates take effect without re-registering.
    for (const r of defaultRenderers(() => currentConfig)) toolRenderers.service.register(r);
