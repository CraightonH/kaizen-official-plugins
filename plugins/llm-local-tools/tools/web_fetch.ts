...
// The `max_bytes` description embeds DEFAULT_CONFIG's value — it does NOT
// track user overrides. Schema is registered once at boot and the `maximum:`
// values stay hardcoded as ceiling guards (CONFIG_SCHEMA constrains the
// configurable defaults below those same ceilings).
export const schema: ToolSchema = {
  ...
  parameters: {
    ...
    properties: {
      ...
      max_bytes:  { type: "integer", minimum: 1024, description: `In-context body cap. Default ${DEFAULT_CONFIG.webFetchCapBytes}. Ignored when save_to is set.` },
...
export function makeHandler(config: LlmLocalToolsConfig) {
  return async function handler(args: WebFetchArgs, ctx: any): Promise<WebFetchResult> {
    let parsed: URL;
    try {
    ...
    const timeout = Math.min(120000, Math.max(1000, args.timeout_ms ?? config.webFetchDefaultTimeoutMs));
    const contextCap = Math.max(1024, args.max_bytes ?? config.webFetchCapBytes);
    ...
    try {
      ...
      if (savePath != null) {
        if (bytes > config.webFetchDownloadCapBytes) {
          throw new Error(
            `web_fetch: response too large to save (${bytes} bytes > ${config.webFetchDownloadCapBytes}): ${parsed.toString()}`
          );
        }
...
// Default handler closure-bound to DEFAULT_CONFIG.
