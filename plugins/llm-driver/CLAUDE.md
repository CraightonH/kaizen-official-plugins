...
loop.ts             runConversation(input, deps) — pure (deps-injected). Owns the LLM call,
                    A-tier single-call path, multi-step strategy/tool loop, and the per-deps
                    WeakMap assemblyCache for prompt:system. Re-emits all `llm:*` and `turn:*`
                    events when it owns the turn (no externalTurnId).
state.ts            CurrentTurn type, snapshotMessages(), aggregateUsage(). Pure.
...
- **Plugin-scoped state, not ctx-scoped.** `setup()` and `start()` receive different `ctx` instances. Mutable state (`currentTurn`, `messages`, `systemPrompt`, `inputHandled`, `exitRequested`, `moduleUi`, `buildDeps`, `depsCache`) lives at module scope. Reset on every `setup()`.
...
- **Turn ownership.** `runConversation` emits `turn:start`/`turn:end` only when `externalTurnId` is undefined. The interactive loop in `index.ts` owns the outer turn and passes `externalTurnId` so loop.ts does not double-emit.
- **Cancellation rolls back.** Before pushing the user message, snapshot the transcript. On AbortError, restore the snapshot and emit `turn:end { reason: "cancelled" }`. Same for non-abort errors → `reason: "error"`.
...
2. Emit from `loop.ts` (per-call events) or `index.ts` (session-/loop-level events). Match the existing naming (`turn:*`, `llm:*`, `conversation:*`, `session:*`).
