...
describe("dts-render", () => {
  it("renders kaizen global with method per tool", async () => {
    const out = await renderDts(simple as unknown as ToolSchema[]);
  ...
  it("non-identifier tool name uses bracket-quoted method", async () => {
    const out = await renderDts(edge as unknown as ToolSchema[]);
  ...
  it("deterministic: same input twice → identical strings", async () => {
    _resetCacheForTest();
    const a = await renderDts(simple as unknown as ToolSchema[]);
    _resetCacheForTest();
    const b = await renderDts(simple as unknown as ToolSchema[]);
    expect(a).toBe(b);
  });
